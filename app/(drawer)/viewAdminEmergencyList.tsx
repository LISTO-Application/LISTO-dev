...
export default function viewAdminEmergencyList({ navigation }: { navigation: any }) {
        ...
        const fetchIncidents = async () => {
          try {
            ...
            const incidentsArray = incidentsSnapshot.docs.map((doc) => ({
              coordinates: doc.data().coordinates || { _latitude: 0, _longitude: 0 }, // Fallback if coordinates are not available
  ...
  const formatCoordinates = (coordinates: { _latitude: number; _longitude: number }): string => {
    ...
    return (
      <View style={styles.mainContainer}>
        ...
        <View style={styles.headerContainer}>
          ...
          <Text style={styles.headerText}>Distress Messages (ADMINS)</Text>
        </View>
          ...
          <ScrollView contentContainerStyle={webstyles.reportList}>
            {incidents.length > 0 ? (
              incidents.map((incident, index) => (
                <View key={index} style={{ marginBottom: 20, padding: 15, borderRadius: 8, backgroundColor: '#f9f9f9', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 3.84 }}>
                  {/* Title and Time Row */}
              ...
              <Text style={{ textAlign: 'center', marginTop: 20 }}>No incidents available.</Text>
            )}
          </ScrollView>
        </View>
      </View>
